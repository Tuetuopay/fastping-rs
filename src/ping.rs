      ...
      loop {
          for (addr, seen) in addrs.lock().unwrap().iter_mut() {
              match if addr.is_ipv4() {
                  ...
                  Err(_e) => {
                      match results_sender.send(PingResult::Unreachable{addr: *addr}) {
                          Err(e) => error!("Error sending ping result on channel: {}", e),
                          _ => {}
                      }
                      // Mark address as seen to avoid sending an Idle result
                      *seen = true;
                      continue
                  },
                  _ => {}
